...
double trapezoides(double a, double b, int n, int thread_count) {
    double local_int[MAX_THREADS] = {0.0};
    double h = (b-a)/n;
    int k;

    #pragma omp parallel num_threads(thread_count) private(k)
    {
        double x;
        ...
        local_int[thread_id] += (f(local_a) + f(local_b)) / 2.0;

        for(k = 1; k <= n_local-1; k++) {
            x = local_a + k * h;
            local_int[thread_id] += f(x);
        }

        local_int[thread_id] *= h;

        printf("El hilo %d ha calculado la integral local [%f,%f]=%f\n", thread_id, local_a, local_b, local_int[thread_id]);
    }

    double integral = 0.0;
    for(k = 0; k < thread_count; k++) {
        integral += local_int[k];
    }
